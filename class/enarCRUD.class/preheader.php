<?php
    ...
    ####################################################################################
    ##
        #infaar we load default sql values from WP config.
	...
	if(!$db){
		echo('Imposible autenticar al usuario. <br />Error: <b>' . mysql_error() . "</b>");
		exit;
	}
	$connect = @mysql_select_db($MYSQL_DB);
	if (!$connect){
		echo('Imposible conectar con la base de datos <br />Error: <b>' . mysql_error() . "</b>");
		exit;
	}
	...
	# what follows are custom database handling functions - required for the enarCRUD class
